...
from kobert_tokenizer import KoBERTTokenizer   # SentencePeice
from transformers import BertModel

kobert_model = BertModel.from_pretrained('skt/kobert-base-v1')
print("KoBERT Model loaded successfully.")
tokenizer = KoBERTTokenizer.from_pretrained('skt/kobert-base-v1')   # Tokenizer add [CLS] and [SEP] tokens automatically by default when encoding
...
class MultiTaskModel(nn.Module):
  def __init__(self, kobert_model, num_label):
    ...
    self.encoder = kobert_model   # a pre-trained model, KoBert
  ...
  def forward(self, input_ids, attention_mask, task):
    # The model learns to output a probability distribution over classes. Index 0 corresponds to class 0 (e.g., "non-hate speech") and index 1 corresponds to class 1 (e.g., "hate speech")
...
num_epoch = 2
mlm_weight = 0.0001  # Starting weight for auto-labeling loss
  ...
  for label_batch, unlabel_batch in zip(train_loader, unlabel_loader):
    ...
    if epoch == num_epoch - 1:
      with torch.no_grad():
        ...
        confidence_score, pseudo_label = torch.max(score, dim=1)   # Returns two paired tensors of each label which are probabilities. Max probability of them will be confidence score and corresponding index will be a label (0 or 1)
...
model_save_path = "/proj/uppmax2024-2-24/hanna/kobert_finetune_v1.pth"
...
pseudo_save_path = "/proj/uppmax2024-2-24/hanna/kobert_pseudo_data.txt"
...
# Re-initialize the model with the same architecture used during fine-tuning
mymodel = MultiTaskModel(kobert_model, num_label)
...
test_save_path = "/proj/uppmax2024-2-24/hanna/kobert_test_result_v1.txt"
...
file_path = "/proj/uppmax2024-2-24/hanna/kobert_test_result_v1.txt"
